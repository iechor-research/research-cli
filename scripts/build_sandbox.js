...
if (!baseImage?.length) {
  console.warn(
    'No default image tag specified in research-cli/packages/cli/package.json',
  );
}
...
console.log('packing @iechor/research-cli-core ...');
const corePackageDir = join('packages', 'core');
rmSync(join(corePackageDir, 'dist', 'iechor-research-cli-core-*.tgz'), {
  force: true,
});
execSync(
  `npm pack -w @iechor/research-cli-core --pack-destination ./packages/core/dist`,
  { stdio: 'ignore' },
);
...
chmodSync(
  join(cliPackageDir, 'dist', `iechor-research-cli-${packageVersion}.tgz`),
  0o755,
);
chmodSync(
...
function buildImage(imageName, dockerfile) {
  ...
  const imageTag =
    process.env.RESEARCH_SANDBOX_IMAGE_TAG || imageName.split(':')[1];
