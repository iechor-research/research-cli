...
function createTempFilesForModify(
  ...
  const diffDir = path.join(tempDir, 'research-cli-tool-modify-diffs');
  ...
  const tempOldPath = path.join(
    diffDir,
    `research-cli-modify-${fileName}-old-${timestamp}${ext}`,
  );
  const tempNewPath = path.join(
    diffDir,
    `research-cli-modify-${fileName}-new-${timestamp}${ext}`,
  );
