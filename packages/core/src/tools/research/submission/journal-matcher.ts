 ...
 * 提供智能期刊推荐、搜索、比较和分析功能
 */
export class JournalMatcher extends BaseResearchTool<
...
> {
  ...
  public validate(params: ResearchToolParams): boolean {
    ...
    switch (matcherParams.action) {
      ...
      case 'recommend':
        return !!(
          matcherParams.title ||
          matcherParams.abstract ||
          matcherParams.keywords
        );
      case 'search':
        return true; // 搜索可以不需要特定参数
      case 'compare':
        return !!(
          matcherParams.journalNames?.length || matcherParams.journalIds?.length
        );
      case 'analyze':
        return true;
      default:
      ...
      [
        {
          ...
          required: true,
          description:
            '执行操作类型 (match, search, compare, analyze, recommend)',
        },
        {
          name: 'title',
          type: 'string',
          required: false,
          description: '论文标题用于匹配',
        },
        {
          name: 'abstract',
          type: 'string',
          required: false,
          description: '论文摘要用于内容分析',
        },
        {
          name: 'keywords',
          type: 'string[]',
        ...
        },
        {
          ...
          type: 'string',
          required: false,
          description: '论文类型 (research, review, survey, etc.)',
        },
        {
          name: 'impactFactorRange',
          type: 'object',
          required: false,
          description: '期望的影响因子范围 {min, max}',
        },
        {
          name: 'quartile',
          type: 'string[]',
          required: false,
          description: 'JCR分区要求 (Q1, Q2, Q3, Q4)',
        },
        {
          name: 'openAccess',
          type: 'boolean',
          required: false,
          description: '偏好开放获取期刊',
        },
        {
          name: 'publisher',
          type: 'string[]',
        ...
        },
        {
          ...
          required: false,
          description:
            '排序标准 (relevance, impact_factor, acceptance_rate, review_time)',
        },
        {
          name: 'maxResults',
          type: 'number',
  ...
  private async compareJournals(
    ...
    if (params.journalNames) {
      for (const name of params.journalNames) {
        const journal = this.journalDatabase.find(
          (j) =>
    ...
    const recommendations =
      this.generateComparisonRecommendations(journalsToCompare);
  ...
  private async analyzeJournals(
    ...
    for (const journal of journals) {
      publisherStats[journal.publisher] =
        (publisherStats[journal.publisher] || 0) + 1;
    }
   ...
   * 计算摘要匹配度
   */
  private calculateAbstractMatch(
  ...
  private calculateKeywordMatch(
    ...
    for (const userKeyword of keywords) {
      ...
      for (const subject of journal.subjects) {
        if (
          userKeyword.toLowerCase().includes(subject.toLowerCase()) ||
   ...
   * 计算影响因子对齐度
   */
  private calculateImpactAlignment(
    params: JournalMatcherParams,
    journal: JournalInfo,
  ): number {
   ...
   * 计算要求匹配度
   */
  private calculateRequirementFit(
    params: JournalMatcherParams,
    journal: JournalInfo,
  ): number {
    let score = 0.5;
  ...
  private sortJournals(
    ...
    const sorted = [...journals].sort((a, b) => {
      ...
      switch (sortBy) {
        ...
        case 'review_time':
          comparison =
            (a.averageReviewTime || 90) - (b.averageReviewTime || 90);
          break;
        case 'relevance':
        default:
          // 基于名称排序作为默认
  ...
  private findSimilarities(journals: JournalInfo[]): string[] {
    ...
    if (impactFactors.length === journals.length) {
      ...
      if (max - min < 2) {
        similarities.push(
          `Similar impact factor range: ${min.toFixed(2)} - ${max.toFixed(2)}`,
        );
      }
    }
  ...
  private findDifferences(journals: JournalInfo[]): string[] {
    ...
    if (impactFactors.length > 1) {
      ...
      if (max - min > 2) {
        differences.push(
          `Significant impact factor variation: ${min.toFixed(2)} to ${max.toFixed(2)}`,
        );
      }
    }

    // 接收率差异
    const acceptanceRates = journals
      .map((j) => j.acceptanceRate)
    ...
    if (acceptanceRates.length > 1) {
      ...
      if (max - min > 20) {
        differences.push(
          `Acceptance rates vary significantly: ${min}% to ${max}%`,
        );
      }
    }

    // 审稿时间差异
    const reviewTimes = journals
      .map((j) => j.averageReviewTime)
  ...
  private generateComparisonRecommendations(journals: JournalInfo[]): string[] {
    ...
    // 根据影响因子推荐
    const withImpactFactor = journals.filter(
      (j) => j.impactFactor !== undefined,
    );
    if (withImpactFactor.length > 1) {
      ...
      recommendations.push(
        `Consider ${highest.name} for highest impact (IF: ${highest.impactFactor})`,
      );
    }

    // 根据接收率推荐
    const withAcceptanceRate = journals.filter(
      (j) => j.acceptanceRate !== undefined,
    );
    if (withAcceptanceRate.length > 1) {
      ...
      recommendations.push(
        `${easiest.name} has the highest acceptance rate (${easiest.acceptanceRate}%)`,
      );
    }

    // 根据审稿时间推荐
    const withReviewTime = journals.filter(
      (j) => j.averageReviewTime !== undefined,
    );
    if (withReviewTime.length > 1) {
      ...
      recommendations.push(
        `${fastest.name} has the fastest review process (${fastest.averageReviewTime} days)`,
      );
    }
  ...
  private generateRecommendations(
    ...
    if (scores.keywordMatch < 0.6) {
      recommendations.push(
        'Review and align keywords with journal focus areas',
      );
    }

    if (journal.wordLimit) {
      recommendations.push(
        `Ensure manuscript meets word limit: ${journal.wordLimit} words`,
      );
    }
    ...
    if (journal.specialIssues && journal.specialIssues.length > 0) {
      recommendations.push(
        `Consider special issues: ${journal.specialIssues.slice(0, 2).join(', ')}`,
      );
    }
  ...
  private generateAnalysisRecommendations(
    ...
    if (impactFactors.length > 0) {
      const avgIF =
        impactFactors.reduce((a, b) => a + b, 0) / impactFactors.length;
      recommendations.push(
        `Average impact factor in this selection: ${avgIF.toFixed(2)}`,
      );

      const q1Journals = journals.filter((j) => j.quartile === 'Q1').length;
      const q1Percentage = (q1Journals / journals.length) * 100;
      recommendations.push(
        `${q1Percentage.toFixed(1)}% of journals are Q1 ranked`,
      );
    }
    ...
    recommendations.push(
      `${oaPercentage.toFixed(1)}% of journals are fully open access`,
    );
  ...
  private initializeJournalDatabase(): void {
    this.journalDatabase = [
      // Computer Science - Top Journals
      {
        ...
        country: 'USA',
        scope:
          'Computer vision, machine learning, pattern recognition, and related fields',
        researchFields: [
          ResearchField.COMPUTER_SCIENCE,
          ResearchField.ENGINEERING,
        ],
      ...
      },
      {
        ...
        country: 'USA',
        scope:
          'Comprehensive surveys and tutorials in all areas of computer science',
      ...
      },
      {
        ...
        country: 'UK',
        scope:
          'Machine intelligence research including algorithms, applications, and societal implications',
        researchFields: [ResearchField.COMPUTER_SCIENCE],
        subjects: [
      ...
      // Engineering Journals
      {
        ...
        country: 'USA',
        scope:
          'Industrial informatics, automation, and intelligent manufacturing systems',
        researchFields: [
          ResearchField.ENGINEERING,
          ResearchField.COMPUTER_SCIENCE,
        ],
      ...
      // Medicine Journals
      {
        ...
        country: 'UK',
        scope:
          'Biomedical research with direct relevance to human health and disease',
      ...
      // Open Access Journals
      {
        ...
        country: 'USA',
        scope:
          'Multidisciplinary research across all areas of science and medicine',
        researchFields: [
          ResearchField.COMPUTER_SCIENCE,
          ResearchField.MEDICINE,
          ResearchField.BIOLOGY,
      ...
      // Mathematics Journals
      {
        ...
        country: 'USA',
        scope:
          'Research articles of the highest quality in all areas of pure mathematics',
