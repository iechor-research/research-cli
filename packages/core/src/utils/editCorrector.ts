...
const EditModel = DEFAULT_RESEARCH_FLASH_MODEL;
...
/**
 * Will look through the research client history and determine when the most recent
 * edit to a target file occured. If no edit happened, it will return -1
 * @param filePath the path to the file
 * @param client the researchClient, so that we can get the history
 ...
 * @param client The ResearchClient for LLM calls.
...
export async function ensureCorrectEdit(
  ...
  const newStringPotentiallyEscaped =
    unescapeStringForResearchBug(originalParams.new_string) !==
  ...
  if (occurrences === expectedReplacements) {
    ...
    const unescapedOldStringAttempt = unescapeStringForResearchBug(
    ...
    if (occurrences === expectedReplacements) {
      ...
      if (llmOldOccurrences === expectedReplacements) {
        ...
        if (newStringPotentiallyEscaped) {
          const baseNewStringForLLMCorrection = unescapeStringForResearchBug(
            originalParams.new_string,
          );
          finalNewString = await correctNewString(
            client,
...
export async function ensureCorrectFileContent(
  ...
  const contentPotentiallyEscaped =
    unescapeStringForResearchBug(content) !== content;
...
`.trim();
  ...
  try {
    const result = await researchClient.generateJson(
      contents,
 ...
 */
export function unescapeStringForResearchBug(inputString: string): string {
  // Regex explanation:
