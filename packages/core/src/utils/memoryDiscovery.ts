...
async function getResearchMdFilePathsInternal(
  ...
  const researchMdFilenames = getAllResearchMdFilenames();

  for (const researchMdFilename of researchMdFilenames) {
    ...
    if (debugMode)
      logger.debug(
        `Searching for ${researchMdFilename} starting from CWD: ${resolvedCwd}`,
      );
    ...
    try {
      ...
      if (debugMode)
        logger.debug(
          `Found readable global ${researchMdFilename}: ${globalMemoryPath}`,
        );
    } catch {
      if (debugMode)
        logger.debug(
          `Global ${researchMdFilename} not found or not readable: ${globalMemoryPath}`,
        );
    }
    ...
    while (currentDir && currentDir !== path.dirname(currentDir)) {
      ...
      if (debugMode) {
        logger.debug(
          `Checking for ${researchMdFilename} in (upward scan): ${currentDir}`,
        );
      }

      // Skip the global .research directory itself during upward scan from CWD,
      // as global is handled separately and explicitly first.
      if (currentDir === path.join(resolvedHome, RESEARCH_CONFIG_DIR)) {
        if (debugMode) {
          logger.debug(
      ...
      const potentialPath = path.join(currentDir, researchMdFilename);
      try {
        ...
        if (potentialPath !== globalMemoryPath) {
          ...
          if (debugMode) {
            logger.debug(
              `Found readable upward ${researchMdFilename}: ${potentialPath}`,
            );
          }
        }
      } catch {
        if (debugMode) {
          logger.debug(
            `Upward ${researchMdFilename} not found or not readable in: ${currentDir}`,
          );
        }
      }
    ...
    if (debugMode && downwardPaths.length > 0)
      logger.debug(
        `Found downward ${researchMdFilename} files (sorted): ${JSON.stringify(
          downwardPaths,
        )}`,
      );
  ...
  if (debugMode)
    logger.debug(
      `Final ordered ${getAllResearchMdFilenames()} paths to read: ${JSON.stringify(
        finalPaths,
      )}`,
    );
  return finalPaths;
...
async function readResearchMdFiles(
  filePaths: string[],
  debugMode: boolean,
): Promise<ResearchFileContent[]> {
  const results: ResearchFileContent[] = [];
...
function concatenateInstructions(
  instructionContents: ResearchFileContent[],
...
/**
 * Loads hierarchical RESEARCH.md files and concatenates their content.
...
export async function loadServerHierarchicalMemory(
  ...
  const filePaths = await getResearchMdFilePathsInternal(
  ...
  if (filePaths.length === 0) {
    if (debugMode) logger.debug('No RESEARCH.md files found in hierarchy.');
    return { memoryContent: '', fileCount: 0 };
  }
  const contentsWithPaths = await readResearchMdFiles(filePaths, debugMode);
