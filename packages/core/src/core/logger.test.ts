...
const TEST_RESEARCH_DIR = path.join(
  os.homedir(),
  RESEARCH_DIR_NAME,
  TMP_DIR_NAME,
  hash,
...
const TEST_LOG_FILE_PATH = path.join(TEST_RESEARCH_DIR, LOG_FILE_NAME);
...
async function cleanupLogAndCheckpointFiles() {
  try {
    await fs.rm(TEST_RESEARCH_DIR, { recursive: true, force: true });
  } catch (_error) {
...
describe('Logger', () => {
  ...
  beforeEach(async () => {
    ...
    await fs.mkdir(TEST_RESEARCH_DIR, { recursive: true });
  ...
  describe('initialize', () => {
    it('should create .research directory and an empty log file if none exist', async () => {
      const dirExists = await fs
        .access(TEST_RESEARCH_DIR)
        .then(() => true)
        .catch(() => false);
    ...
    it('should handle invalid JSON in log file by backing it up and starting fresh', async () => {
      ...
      const logContent = await readLogFile();
      expect(logContent).toEqual([]);
      const dirContents = await fs.readdir(TEST_RESEARCH_DIR);
      expect(
        dirContents.some(
          (f) =>
