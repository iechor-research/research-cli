...
describe('useSlashCommandProcessor', () => {
  ...
  beforeEach(() => {
    ...
    mockConfig = {
      ...
      getResearchClient: () => mockResearchClient,
  ...
  describe('/bug command', () => {
    ...
    const getExpectedUrl = (
      ...
      if (sandboxEnvVar && sandboxEnvVar !== 'sandbox-exec') {
        sandboxEnvStr = sandboxEnvVar.replace(/^research-(?:code-)?/, '');
...
`;
      let url =
        'https://github.com/iechor-research/research-cli/issues/new?template=bug_report.yml';
      if (description) {
...
`;
  ...
  describe('/tools command', () => {
    ...
    it('should display only Research CLI tools (filtering out MCP tools)', async () => {
      // Create mock tools - some with serverName property (MCP tools) and some without (Research CLI tools)
      const mockTools = [
    ...
    it('should display a message when no Research CLI tools are available', async () => {
  ...
  describe('/mcp command', () => {
    ...
    it('should display a message and open a URL when no MCP servers are configured outside a sandbox', async () => {
      ...
      expect(mockAddItem).toHaveBeenNthCalledWith(
        ...
        expect.objectContaining({
          type: MessageType.INFO,
          text: 'No MCP servers configured. Opening documentation in your browser: https://goo.gle/research-cli-docs-mcp',
        }),
        expect.any(Number),
      );
  ...
  describe('/compress command', () => {
    it('should call tryCompressChat(true)', async () => {
      ...
      expect(mockResearchClient.tryCompressChat).toHaveBeenCalledWith(
        'Prompt Id not set',
        true,
      );
      expect(mockAddItem).toHaveBeenNthCalledWith(
