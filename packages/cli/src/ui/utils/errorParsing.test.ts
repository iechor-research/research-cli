...
describe('parseAndFormatApiError', () => {
  ...
  it('should format a nested API error', () => {
    const nestedErrorMessage = JSON.stringify({
      error: {
        code: 429,
        message:
          "Research 2.5 Pro Preview doesn't have a free quota tier. For more information on this error, head to: https://ai.iechor.dev/research-api/docs/rate-limits.",
  ...
  it('should not match non-Pro models with similar version strings', () => {
    ...
    expect(
      isProQuotaExceededError(
        "Quota exceeded for quota metric 'Research 2.5 Flash Requests' and limit",
      ),
    ).toBe(false);
    expect(
      isProQuotaExceededError(
        "Quota exceeded for quota metric 'Research 2.5-preview Flash Requests' and limit",
      ),
    ).toBe(false);
    ...
    expect(
      isProQuotaExceededError(
        "Quota exceeded for quota metric 'Research 2.5 Ultra Requests' and limit",
      ),
    ).toBe(false);
    expect(
      isProQuotaExceededError(
        "Quota exceeded for quota metric 'Research 2.5 Standard Requests' and limit",
      ),
    ).toBe(false);
