...
export const SETTINGS_DIRECTORY_NAME = '.research';
export const USER_SETTINGS_DIR = path.join(homedir(), SETTINGS_DIRECTORY_NAME);
export const USER_SETTINGS_PATH = path.join(USER_SETTINGS_DIR, 'settings.json');

function getSystemSettingsPath(): string {
  if (platform() === 'darwin') {
    return '/Library/Application Support/ResearchCli/settings.json';
  } else if (platform() === 'win32') {
    return 'C:\\ProgramData\\research-cli\\settings.json';
  } else {
    return '/etc/research-cli/settings.json';
  }
}
