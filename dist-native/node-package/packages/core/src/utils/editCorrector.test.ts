...
vi.mock('../core/client.js', () => ({
  ResearchClient: vi.fn().mockImplementation(function (
    this: any,
    _config: Config,
  ) {
...
describe('editCorrector', () => {
  ...
  describe('unescapeStringForResearchBug', () => {
    it('should unescape common sequences', () => {
      expect(unescapeStringForResearchBug('\\n')).toBe('\n');
      expect(unescapeStringForResearchBug('\\t')).toBe('\t');
      expect(unescapeStringForResearchBug("\\'")).toBe("'");
      expect(unescapeStringForResearchBug('\\"')).toBe('"');
      expect(unescapeStringForResearchBug('\\`')).toBe('`');
    });
    it('should handle multiple escaped sequences', () => {
      expect(unescapeStringForResearchBug('Hello\\nWorld\\tTest')).toBe(
        'Hello\nWorld\tTest',
      );
    });
    it('should not alter already correct sequences', () => {
      expect(unescapeStringForResearchBug('\n')).toBe('\n');
      expect(unescapeStringForResearchBug('Correct string')).toBe(
        'Correct string',
      );
    });
    ...
    it('should handle backslash followed by actual newline character', () => {
      expect(unescapeStringForResearchBug('\\\n')).toBe('\n');
      expect(unescapeStringForResearchBug('First line\\\nSecond line')).toBe(
        'First line\nSecond line',
      );
    });
    ...
    it('should not alter strings with no targeted escape sequences', () => {
      expect(unescapeStringForResearchBug('abc def')).toBe('abc def');
      expect(unescapeStringForResearchBug('C:\\Folder\\File')).toBe(
        'C:\\Folder\\File',
      );
    });
    it('should correctly process strings with some targeted escapes', () => {
      expect(unescapeStringForResearchBug('C:\\Users\\name')).toBe(
        'C:\\Users\name',
      );
    });
    ...
    it('should handle escaped backslashes', () => {
      expect(unescapeStringForResearchBug('\\\\')).toBe('\\');
      expect(unescapeStringForResearchBug('C:\\\\Users')).toBe('C:\\Users');
      expect(unescapeStringForResearchBug('path\\\\to\\\\file')).toBe(
        'path\to\\file',
      );
    });
    it('should handle escaped backslashes mixed with other escapes (aggressive unescaping)', () => {
      expect(unescapeStringForResearchBug('line1\\\\\\nline2')).toBe(
        'line1\nline2',
      );
      expect(unescapeStringForResearchBug('quote\\\\"text\\\\nline')).toBe(
        'quote"text\nline',
      );
    });
  });
  ...
  describe('ensureCorrectEdit', () => {
    let mockResearchClientInstance: Mocked<ResearchClient>;
    ...
    beforeEach(() => {
      ...
      mockConfigInstance = {
        ...
        getResearchMdFileCount: vi.fn(() => configParams.researchMdFileCount),
        setResearchMdFileCount: vi.fn((count: number) => {
          configParams.researchMdFileCount = count;
        }),
      ...
      mockResearchClientInstance = new ResearchClient(
        mockConfigInstance,
      ) as Mocked<ResearchClient>;
      mockResearchClientInstance.getHistory = vi.fn().mockResolvedValue([]);
      resetEditCorrectorCaches_TEST_ONLY();
    });

    describe('Scenario Group 1: originalParams.old_string matches currentContent directly', () => {
      it('Test 1.1: old_string (no literal \\), new_string (escaped by Research) -> new_string unescaped', async () => {
      ...
      it('Test 1.3: old_string (with literal \\), new_string (escaped by Research) -> new_string unchanged (still escaped)', async () => {
        const currentContent = 'This is a test string to find\\me.';
        const originalParams = {
          file_path: '/test/file.txt',
          old_string: 'find\\me',
    ...
    describe('Scenario Group 2: originalParams.old_string does NOT match, but unescapeStringForResearchBug(originalParams.old_string) DOES match', () => {
      it('Test 2.1: old_string (over-escaped, no intended literal \\), new_string (escaped by Research) -> new_string unescaped', async () => {
        const currentContent = 'This is a test string to find "me".';
        const originalParams = {
          file_path: '/test/file.txt',
          old_string: 'find \\"me\\"',
    ...
    describe('Scenario Group 3: LLM Correction Path', () => {
      it('Test 3.1: old_string (no literal \\), new_string (escaped by Research), LLM re-escapes new_string -> final new_string is double unescaped', async () => {
      ...
      it('Test 3.2: old_string (with literal \\), new_string (escaped by Research), LLM re-escapes new_string -> final new_string is unescaped once', async () => {
        const currentContent = 'This is a test string to corrected find me.';
        const originalParams = {
          file_path: '/test/file.txt',
          old_string: 'find\\me',
    ...
    describe('Scenario Group 5: Specific unescapeStringForResearchBug checks (integrated into ensureCorrectEdit)', () => {
    ...
    describe('Scenario Group 6: Concurrent Edits', () => {
      it('Test 6.1: should return early if file was modified by another process', async () => {
        ...
        (mockResearchClientInstance.getHistory as Mock).mockResolvedValue(
          history,
        );
