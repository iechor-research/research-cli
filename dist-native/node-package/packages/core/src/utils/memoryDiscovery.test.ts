...
const ORIGINAL_RESEARCH_MD_FILENAME_CONST_FOR_TEST = DEFAULT_CONTEXT_FILENAME;
...
describe('loadServerHierarchicalMemory', () => {
  ...
  let GLOBAL_RESEARCH_DIR: string;
  let GLOBAL_RESEARCH_FILE: string; // Defined in beforeEach
  ...
  beforeEach(() => {
    ...
    setResearchMdFilename(DEFAULT_CONTEXT_FILENAME); // Use defined const
    mockOs.homedir.mockReturnValue(USER_HOME);

    // Define these here to use potentially reset/updated values from imports
    GLOBAL_RESEARCH_DIR = path.join(USER_HOME, RESEARCH_CONFIG_DIR);
    GLOBAL_RESEARCH_FILE = path.join(
      GLOBAL_RESEARCH_DIR,
      getCurrentResearchMdFilename(), // Use current filename
    );
  ...
  it('should load only the global custom context file if present and filename is changed', async () => {
    ...
    setResearchMdFilename(customFilename);
    const globalCustomFile = path.join(GLOBAL_RESEARCH_DIR, customFilename);
  ...
  it('should load ORIGINAL_RESEARCH_MD_FILENAME files by upward traversal from CWD to project root', async () => {
    const projectRootResearchFile = path.join(
      PROJECT_ROOT,
      ORIGINAL_RESEARCH_MD_FILENAME_CONST_FOR_TEST,
    );
    const srcResearchFile = path.join(
      CWD,
      ORIGINAL_RESEARCH_MD_FILENAME_CONST_FOR_TEST,
    );
    ...
    mockFs.access.mockImplementation(async (p) => {
      if (p === projectRootResearchFile || p === srcResearchFile) {
        return undefined;
      }
      throw new Error('File not found');
    });
    ...
    mockFs.readFile.mockImplementation(async (p) => {
      if (p === projectRootResearchFile) {
        return 'Project root memory';
      }
      if (p === srcResearchFile) {
    ...
    const expectedContent =
      `--- Context from: ${path.relative(CWD, projectRootResearchFile)} ---\nProject root memory\n--- End of Context from: ${path.relative(CWD, projectRootResearchFile)} ---\n\n` +
      `--- Context from: ${ORIGINAL_RESEARCH_MD_FILENAME_CONST_FOR_TEST} ---\nSrc directory memory\n--- End of Context from: ${ORIGINAL_RESEARCH_MD_FILENAME_CONST_FOR_TEST} ---`;

    expect(memoryContent).toBe(expectedContent);
    expect(fileCount).toBe(2);
    expect(mockFs.readFile).toHaveBeenCalledWith(
      projectRootResearchFile,
      'utf-8',
    );
    expect(mockFs.readFile).toHaveBeenCalledWith(srcResearchFile, 'utf-8');
  });

  it('should load ORIGINAL_RESEARCH_MD_FILENAME files by downward traversal from CWD', async () => {
    const subDir = path.join(CWD, 'subdir');
    const subDirResearchFile = path.join(
      subDir,
      ORIGINAL_RESEARCH_MD_FILENAME_CONST_FOR_TEST,
    );
    const cwdResearchFile = path.join(
      CWD,
      ORIGINAL_RESEARCH_MD_FILENAME_CONST_FOR_TEST,
    );

    mockFs.access.mockImplementation(async (p) => {
      if (p === cwdResearchFile || p === subDirResearchFile) return undefined;
      throw new Error('File not found');
    });

    mockFs.readFile.mockImplementation(async (p) => {
      if (p === cwdResearchFile) return 'CWD memory';
      if (p === subDirResearchFile) return 'Subdir memory';
      throw new Error('File not found');
    });
    ...
    mockFs.readdir.mockImplementation((async (
      ...
      if (p === CWD) {
        return [
          {
            name: ORIGINAL_RESEARCH_MD_FILENAME_CONST_FOR_TEST,
            isFile: () => true,
            isDirectory: () => false,
          } as Dirent,
          {
    ...
    const expectedContent =
      `--- Context from: ${ORIGINAL_RESEARCH_MD_FILENAME_CONST_FOR_TEST} ---\nCWD memory\n--- End of Context from: ${ORIGINAL_RESEARCH_MD_FILENAME_CONST_FOR_TEST} ---\n\n` +
      `--- Context from: ${path.join('subdir', ORIGINAL_RESEARCH_MD_FILENAME_CONST_FOR_TEST)} ---\nSubdir memory\n--- End of Context from: ${path.join('subdir', ORIGINAL_RESEARCH_MD_FILENAME_CONST_FOR_TEST)} ---`;

    expect(memoryContent).toBe(expectedContent);
    expect(fileCount).toBe(2);
  });

  it('should load and correctly order global, upward, and downward ORIGINAL_RESEARCH_MD_FILENAME files', async () => {
    setResearchMdFilename(ORIGINAL_RESEARCH_MD_FILENAME_CONST_FOR_TEST); // Explicitly set for this test
    ...
    const projectParentResearchFile = path.join(
      projectParentDir,
      ORIGINAL_RESEARCH_MD_FILENAME_CONST_FOR_TEST,
    );
    const projectRootResearchFile = path.join(
    ...
    mockFs.stat.mockImplementation(async (p) => {
      if (p === path.join(PROJECT_ROOT, '.git')) {
        return { isDirectory: () => true } as Stats;
      } else if (p === path.join(PROJECT_ROOT, '.research')) {
        return { isDirectory: () => true } as Stats;
      }
      throw new Error('File not found');
    });
    ...
    mockFs.access.mockImplementation(async (p) => {
      if (
        ...
        p === projectParentResearchFile ||
        p === projectRootResearchFile ||
        p === cwdResearchFile ||
        p === subDirResearchFile
      ) {
        return undefined;
      }
      throw new Error('File not found');
    ...
    mockFs.readFile.mockImplementation(async (p) => {
      ...
      if (p === projectParentResearchFile) return 'Project parent memory';
      if (p === projectRootResearchFile) return 'Project root memory';
      if (p === cwdResearchFile) return 'CWD memory';
      if (p === subDirResearchFile) return 'Subdir memory';
      throw new Error('File not found');
    });
    ...
    const relPathGlobal = path.relative(CWD, GLOBAL_RESEARCH_FILE);
    const relPathProjectParent = path.relative(CWD, projectParentResearchFile);
    const relPathProjectRoot = path.relative(CWD, projectRootResearchFile);
    const relPathCwd = ORIGINAL_RESEARCH_MD_FILENAME_CONST_FOR_TEST;
  ...
  it('should ignore specified directories during downward scan', async () => {
    ...
    const ignoredDirResearchFile = path.join(
      ignoredDir,
      ORIGINAL_RESEARCH_MD_FILENAME_CONST_FOR_TEST,
    ); // Corrected
    const regularSubDir = path.join(CWD, 'my_code');
    const regularSubDirResearchFile = path.join(
      regularSubDir,
      ORIGINAL_RESEARCH_MD_FILENAME_CONST_FOR_TEST,
    );

    mockFs.access.mockImplementation(async (p) => {
      if (p === regularSubDirResearchFile) return undefined;
      if (p === ignoredDirResearchFile)
        throw new Error('Should not access ignored file');
      throw new Error('File not found');
    });

    mockFs.readFile.mockImplementation(async (p) => {
      if (p === regularSubDirResearchFile) return 'My code memory';
      throw new Error('File not found');
    });
    ...
    const expectedContent = `--- Context from: ${path.join('my_code', ORIGINAL_RESEARCH_MD_FILENAME_CONST_FOR_TEST)} ---\nMy code memory\n--- End of Context from: ${path.join('my_code', ORIGINAL_RESEARCH_MD_FILENAME_CONST_FOR_TEST)} ---`;

    expect(memoryContent).toBe(expectedContent);
    expect(fileCount).toBe(1);
    expect(mockFs.readFile).not.toHaveBeenCalledWith(
      ignoredDirResearchFile,
      'utf-8',
    );
  });
  ...
  it('should load extension context file paths', async () => {
    const extensionFilePath = '/test/extensions/ext1/RESEARCH.md';
    mockFs.access.mockImplementation(async (p) => {
