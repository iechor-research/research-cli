...
import { setResearchMdFilename as mockSetResearchMdFilename } from '../tools/memoryTool.js';
import {
...
vi.mock('../core/client.js', () => ({
  ResearchClient: vi.fn().mockImplementation(() => ({
...
describe('Server Config (config.ts)', () => {
  const MODEL = 'research-pro';
  const SANDBOX: SandboxConfig = {
    command: 'docker',
    image: 'research-cli-sandbox',
  };
  ...
  const EMBEDDING_MODEL = 'research-embedding';
  ...
  describe('refreshAuth', () => {
    it('should refresh auth and update config', async () => {
      ...
      expect(createContentGeneratorConfig).toHaveBeenCalledWith(
        MODEL, // Should be called with the original model 'research-pro'
        authType,
      );
      ...
      expect(ResearchClient).toHaveBeenCalledWith(config);
    });
  });
  ...
  it('Config constructor should call setResearchMdFilename with contextFileName if provided', () => {
    ...
    expect(mockSetResearchMdFilename).toHaveBeenCalledWith(contextFileName);
  });

  it('Config constructor should not call setResearchMdFilename if contextFileName is not provided', () => {
    new Config(baseParams); // baseParams does not have contextFileName
    expect(mockSetResearchMdFilename).not.toHaveBeenCalled();
  });
  ...
  describe('refreshMemory', () => {
    it('should update memory and file count on successful refresh', async () => {
      ...
      expect(config.getResearchMdFileCount()).toBe(mockMemoryData.fileCount);
