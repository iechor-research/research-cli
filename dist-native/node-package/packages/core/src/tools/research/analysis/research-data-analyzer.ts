 ...
 */
export class ResearchDataAnalyzer extends BaseResearchTool<
  DataAnalyzerParams,
  DataAnalysisResult
> {
  constructor() {
  ...
  public validate(params: ResearchToolParams): boolean {
    ...
    if (
      !p.action ||
      !['analyze', 'visualize', 'report', 'preprocess', 'explore'].includes(
        p.action,
      )
    ) {
      return false;
    ...
    if (
      !p.analysisTypes ||
      !Array.isArray(p.analysisTypes) ||
      p.analysisTypes.length === 0
    ) {
      return false;
    }
...
高级选项：
  ...
  protected async executeImpl(
    ...
    switch (params.action) {
      ...
      case 'explore':
        // 探索性数据分析
        const exploratoryResult = await this.performExploratoryAnalysis(
          data,
          params,
        );
  ...
  private async loadData(
    ...
    if (params.dataContent) {
      try {
        ...
        throw new Error(
          `Failed to parse data content: ${error instanceof Error ? error.message : 'Unknown error'}`,
        );
      }
    }
  ...
  private async performDescriptiveAnalysis(
    ...
    if (summary.numericColumns.length > 1) {
      correlationAnalysis = this.calculateCorrelations(
        ...
        params.correlationMethod || 'pearson',
      );
    ...
    const recommendations = this.generateDescriptiveRecommendations(
      descriptiveStats,
      summary,
    );
  ...
  private async performInferentialAnalysis(
    ...
    if (
      params.targetColumn &&
      summary.numericColumns.includes(params.targetColumn)
    ) {
      for (const column of summary.numericColumns) {
        if (column !== params.targetColumn) {
          const correlationTest = this.correlationSignificanceTest(
            data,
            column,
            params.targetColumn,
          );
    ...
    if (
      ...
      const leveneTest = this.leveneTest(
        data,
    ...
    const recommendations =
      this.generateInferentialRecommendations(hypothesisTests);
  ...
  private async performMLAnalysis(
    ...
    if (summary.numericColumns.length >= 2) {
      const clusteringResult = this.performClustering(
        data,
        summary.numericColumns,
        params.clusteringMethod || 'kmeans',
      );
    ...
    if (params.targetColumn) {
      const superviseResult = this.performSupervisedLearning(
        data,
        params.targetColumn,
        summary.numericColumns,
        params.regressionMethod || 'linear',
      );
  ...
  private async performTimeSeriesAnalysis(
    ...
    const timeSeriesAnalysis = this.analyzeTimeSeries(
      data,
      params.timeColumn,
      params.targetColumn,
    );
    ...
    if (params.includeVisualization) {
      ...
      visualizations.push({
        ...
        config: {
          xlabel: params.timeColumn,
          ylabel: params.targetColumn || 'Index',
        },
    ...
    const recommendations =
      this.generateTimeSeriesRecommendations(timeSeriesAnalysis);
  ...
  private async performExploratoryAnalysis(
    ...
    recommendations.push(
      `数据集包含 ${summary.totalRows} 行和 ${summary.totalColumns} 列`,
    );
    recommendations.push(
      `数值列: ${summary.numericColumns.length} 个，分类列: ${summary.categoricalColumns.length} 个`,
    );
    ...
    if (missingColumns.length > 0) {
      recommendations.push(
        `发现缺失值：${missingColumns.map(([col, count]) => `${col}(${count})`).join(', ')}`,
      );
    ...
    // 数据分布分析
    for (const column of summary.numericColumns.slice(0, 5)) {
      // 限制前5个
      const values = data
        .map((row) => Number(row[column]))
        .filter((val) => !isNaN(val));
    ...
    if (summary.numericColumns.length > 1) {
      ...
      const strongCorrelations = correlationAnalysis.significantPairs.filter(
        (pair) =>
  ...
  private async generateVisualizations(
    ...
    // 相关性热力图
    if (summary.numericColumns.length > 1) {
      const correlationMatrix = this.calculateCorrelations(
        data,
        summary.numericColumns,
        'pearson',
      );
    ...
    // 散点图矩阵（如果变量不太多）
    if (
      summary.numericColumns.length >= 2 &&
      summary.numericColumns.length <= 4
    ) {
  ...
  private async preprocessData(
    ...
    if (missingColumns.length > 0) {
      ...
      missingColumns.forEach(([column, count]) => {
        ...
        if (percentage > 50) {
          recommendations.push(
            `- ${column}: 缺失值过多(${percentage.toFixed(1)}%)，建议考虑删除该列`,
          );
        } else if (percentage > 10) {
          recommendations.push(
            `- ${column}: 缺失值较多(${percentage.toFixed(1)}%)，建议使用插值或建模填充`,
          );
        } else {
          recommendations.push(
            `- ${column}: 缺失值较少(${percentage.toFixed(1)}%)，可以删除这些行或用均值/众数填充`,
          );
        }
      });
    }
    ...
    for (const column of summary.numericColumns) {
      ...
      if (outliers.length > 0) {
        ...
        recommendations.push(
          `- ${column} 列发现 ${outliers.length} 个异常值(${outlierPercentage.toFixed(1)}%)`,
        );
    ...
    for (const column of summary.categoricalColumns) {
      ...
      if (uniqueValues < 10) {
        recommendations.push(
          `- ${column}: 可以转换为分类变量(${uniqueValues}个唯一值)`,
        );
      } else if (uniqueValues > summary.totalRows * 0.8) {
        recommendations.push(
          `- ${column}: 唯一值过多，可能是标识符，考虑在分析中排除`,
        );
      }
    }
...
<body>
  ...
  private calculateSkewness(
    ...
    const skewness =
      values.reduce((sum, val) => sum + Math.pow((val - mean) / std, 3), 0) / n;
    return skewness;
  }
  ...
  private calculateKurtosis(
    ...
    const n = values.length;
    const kurtosis =
      values.reduce((sum, val) => sum + Math.pow((val - mean) / std, 4), 0) /
        n -
      3;
    return kurtosis;
  }
  ...
  private tDistribution(t: number, df: number): number {
    // 简化的t分布CDF近似
    return (
      0.5 + 0.5 * Math.sign(t) * Math.sqrt(1 - Math.exp((-2 * t * t) / Math.PI))
    );
  }

  private getSignificanceLevel(
    pValue: number,
  ...
  private generateDescriptiveRecommendations(
    ...
    stats.forEach((stat) => {
      if (stat.skewness && Math.abs(stat.skewness) > 1) {
        recommendations.push(
          `${stat.column} 分布偏度较大(${stat.skewness.toFixed(2)})，建议考虑对数变换`,
        );
      }
      ...
      if (
        stat.uniqueValues &&
        stat.count &&
        stat.uniqueValues / stat.count < 0.1
      ) {
        recommendations.push(
          `${stat.column} 唯一值比例低，可能适合作为分类变量`,
        );
      }
    });
  ...
  private generateInferentialRecommendations(
    tests: HypothesisTestResult[],
  ): string[] {
    const recommendations: string[] = [];

    const normalityTests = tests.filter((t) => t.testType === 'normality');
    const nonNormalCount = normalityTests.filter(
      (t) => t.conclusion === 'reject_null',
    ).length;

    if (nonNormalCount > 0) {
      recommendations.push(
        `${nonNormalCount} 个变量不符合正态分布，建议使用非参数检验`,
      );
    }

    const correlationTests = tests.filter(
      (t) => t.testType === 'ttest' && t.testName.includes('相关性'),
    );
    const significantCorrelations = correlationTests.filter(
      (t) => t.conclusion === 'reject_null',
    ).length;

    if (significantCorrelations > 0) {
      recommendations.push(
        `发现 ${significantCorrelations} 对变量存在显著相关性`,
      );
    }
  ...
  private generateMLRecommendations(results: MLAnalysisResult[]): string[] {
    ...
    results.forEach((result) => {
      if (result.analysisType === 'clustering') {
        ...
        result.analysisType === 'regression' &&
        result.results.r2Score
      ) {
  ...
  private generateTimeSeriesRecommendations(
    ...
    if (analysis.seasonality.detected) {
      recommendations.push(
        `检测到季节性模式，周期约为 ${analysis.seasonality.period}`,
      );
    }
