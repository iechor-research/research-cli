...
const MEMORY_SECTION_HEADER = '## Research Added Memories';
...
describe('MemoryTool', () => {
  ...
  describe('setResearchMdFilename', () => {
    it('should update currentResearchMdFilename when a valid new name is provided', () => {
      const newName = 'CUSTOM_CONTEXT.md';
      setResearchMdFilename(newName);
      expect(getCurrentResearchMdFilename()).toBe(newName);
    });

    it('should not update currentResearchMdFilename if the new name is empty or whitespace', () => {
      const initialName = getCurrentResearchMdFilename(); // Get current before trying to change
      setResearchMdFilename('  ');
      expect(getCurrentResearchMdFilename()).toBe(initialName);
    ...
    it('should handle an array of filenames', () => {
      ...
      setResearchMdFilename(newNames);
      expect(getCurrentResearchMdFilename()).toBe('CUSTOM_CONTEXT.md');
      expect(getAllResearchMdFilenames()).toEqual(newNames);
    });
  });
  ...
  describe('execute (instance method)', () => {
    ...
    it('should call performAddMemoryEntry with correct parameters and return success', async () => {
      ...
      // Use getCurrentResearchMdFilename for the default expectation before any setResearchMdFilename calls in a test
      const expectedFilePath = path.join(
        '/mock/home',
        '.research',
        getCurrentResearchMdFilename(), // This will be DEFAULT_CONTEXT_FILENAME unless changed by a test
      );
