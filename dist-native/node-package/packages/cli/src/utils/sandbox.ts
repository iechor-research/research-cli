...
import { SandboxConfig } from '@iechor/research-cli-core';
...
const LOCAL_DEV_SANDBOX_IMAGE_NAME = 'research-cli-sandbox';
const SANDBOX_NETWORK_NAME = 'research-cli-sandbox';
const SANDBOX_PROXY_NAME = 'research-cli-sandbox-proxy';
...
function entrypoint(workdir: string): string[] {
  ...
  const cliCmd =
    process.env.NODE_ENV === 'development'
      ...
      : process.env.DEBUG
        ? `node --inspect-brk=0.0.0.0:${process.env.DEBUG_PORT || '9229'} $(which research)`
        : 'research';
...
export async function start_sandbox(
  ...
  // determine full path for research-cli to distinguish linked vs installed setting
  ...
  // if BUILD_SANDBOX is set, then call scripts/build_sandbox.js under research-cli repo
  //
  // note this can only be done with binary linked from research-cli repo
  if (process.env.BUILD_SANDBOX) {
    if (!gcPath.includes('research-cli/packages/')) {
      console.error(
        'ERROR: cannot build sandbox using installed research binary; ' +
          'run `npm link ./packages/cli` under research-cli repo to switch to linked binary.',
      );
      process.exit(1);
    } else {
  ...
  if (!(await ensureSandboxImageIsPresent(config.command, image))) {
    const remedy =
      image === LOCAL_DEV_SANDBOX_IMAGE_NAME
        ? 'Try running `npm run build:all` or `npm run build:sandbox` under the research-cli repo to build it locally, or check the image name and your network connection.'
        : 'Please check the image name, your network connection, or notify research-cli-dev@iechor.com if the issue persists.';
    console.error(
  ...
  if (process.env.RESEARCH_CLI_INTEGRATION_TEST === 'true') {
    ...
    // finally switch to that user to run the research process. This is
    // necessary on Linux to ensure the user exists within the
    // container's /etc/passwd file, which is required by os.userInfo().
    const username = 'research';
