...
export const useSlashCommandProcessor = (
  ...
  const savedChatTags = useCallback(async () => {
    const researchDir = config?.getProjectTempDir();
    if (!researchDir) {
      return [];
    }
    try {
  ...
  const legacyCommands: LegacySlashCommand[] = useMemo(() => {
    const commands: LegacySlashCommand[] = [
      ...
      {
        name: 'docs',
        description: 'open full Research CLI documentation in your browser',
        action: async (_mainCommand, _subCommand, _args) => {
          const docsUrl = 'https://goo.gle/research-cli-docs';
      ...
      },
      {
        ...
        action: async (_mainCommand, _subCommand, _args) => {
          ...
          if (serverNames.length === 0) {
            const docsUrl = 'https://goo.gle/research-cli-docs-mcp';
            if (process.env.SANDBOX && process.env.SANDBOX !== 'sandbox-exec') {
              addMessage({
                type: MessageType.INFO,
      ...
      {
        name: 'tools',
        description: 'list available Research CLI tools',
        action: async (_mainCommand, _subCommand, _args) => {
          // Check if the _subCommand includes a specific flag to control description visibility
          let useShowDescriptions = showToolDescriptions;
          if (_subCommand === 'desc' || _subCommand === 'descriptions') {
...
`;
          ...
          let bugReportUrl =
            'https://github.com/iechor-research/research-cli/issues/new?template=bug_report.yml&title={title}&info={info}';
      ...
      },
      {
        ...
        action: async (_mainCommand, subCommand, args) => {
          ...
          const chat = await config?.getResearchClient()?.getChat();
          if (!chat) {
            addMessage({
              type: MessageType.ERROR,
      ...
      commands.push({
        ...
        action: async (_mainCommand, subCommand, _args) => {
          ...
          if (!checkpointDir) {
            addMessage({
              type: MessageType.ERROR,
              content: 'Could not determine the .research directory path.',
              timestamp: new Date(),
            });
            return;
          }
