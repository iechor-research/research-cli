...
export function formatTable(
  ...
  columns: TableColumn[], 
  config: TableConfig = { showHeaders: true, showBorders: true, colorScheme: 'default' }
): string {
  if (data.length === 0) {
  ...
  if (config.showBorders) {
    result += colorize(createBorderRow(columnWidths, '┌', '┬', '┐'), borderColor) + '\n';
  }
  ...
  if (config.showHeaders) {
    ...
    result += colorize('│ ', borderColor) + headerRow + colorize(' │', borderColor) + '\n';
  ...
  for (const row of data) {
    ...
    result += colorize('│ ', borderColor) + dataRow + colorize(' │', borderColor) + '\n';
  }
...
export function formatKeyValue(
  ...
  return Object.entries(data).map(([key, value]) => 
    colorize(key + ':', keyColor) + ' ' + colorize(String(value), valueColor)
  ).join('\n');
}
 ...
 * 辅助函数：创建边框行
 */
function createBorderRow(widths: number[], left: string, middle: string, right: string): string {
 ...
 * 辅助函数：根据配色方案获取颜色
 */
function getColorByScheme(scheme: 'default' | 'accent' | 'muted', type: 'border' | 'header' | 'data'): string {
  switch (scheme) {
    case 'accent':
      return type === 'header' ? Colors.Cyan : type === 'border' ? Colors.Blue : Colors.Reset;
    case 'muted':
      return Colors.Dim;
    default:
