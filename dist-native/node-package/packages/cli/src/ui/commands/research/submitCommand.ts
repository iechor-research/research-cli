...
export const submitCommand: SlashCommand = {
  name: 'submit',
  description: 'Submission preparation tools for journal matching, package preparation, and LaTeX management',
  subCommands: [
    {
      name: 'match',
      ...
      action: async (context: CommandContext, args: string): Promise<void | SlashCommandActionReturn> => {
        try {
          ...
          const field = getOptionValue(parsed.options, 'field', 'COMPUTER_SCIENCE') as string;
          const maxResults = getOptionValue(parsed.options, 'limit', 5);

          validateOption(field, ['COMPUTER_SCIENCE', 'BIOLOGY', 'PHYSICS', 'MATHEMATICS', 'ENGINEERING'], 'field');

          context.ui.addItem({
            type: MessageType.INFO,
            text: formatInfo(`Matching journals for paper in ${field.toLowerCase()} (showing top ${maxResults} matches)`)
          }, Date.now());
...

    {
      ...
      action: async (context: CommandContext, args: string): Promise<void | SlashCommandActionReturn> => {
        try {
          ...
          const format = getOptionValue(parsed.options, 'format', 'pdf') as string;
          const outputDir = getOptionValue(parsed.options, 'output', './submission') as string;

          validateOption(format, ['pdf', 'latex', 'word'], 'format');

          context.ui.addItem({
            type: MessageType.INFO,
            text: formatInfo(`Preparing ${format} submission package for ${journal}`)
          }, Date.now());
          ...
          return {
            ...
            toolArgs: {
              ...
              includeSupplementary: getOptionValue(parsed.options, 'supplementary', true),
...

    {
      ...
      action: async (context: CommandContext, args: string): Promise<void | SlashCommandActionReturn> => {
        try {
          ...
          validateOption(operation, ['create', 'compile', 'clean', 'check', 'template', 'package'], 'operation');
          ...
          switch (operation) {
            case 'create':
              toolArgs.template = getOptionValue(parsed.options, 'template', 'ieee_conference') as string;
              toolArgs.title = getOptionValue(parsed.options, 'title', 'Untitled Paper') as string;
              toolArgs.authors = getOptionValue(parsed.options, 'authors', 'Author Name') as string;
              break;

            case 'compile':
              toolArgs.engine = getOptionValue(parsed.options, 'engine', 'PDFLATEX') as string;
              toolArgs.bibtex = getOptionValue(parsed.options, 'bibtex', true);
              toolArgs.clean = getOptionValue(parsed.options, 'clean', false);
              break;

            case 'template':
              toolArgs.templateType = getOptionValue(parsed.options, 'type', 'ieee_conference') as string;
              validateOption(toolArgs.templateType, ['ieee_conference', 'acm_article', 'springer_journal', 'thesis'], 'type');
              break;

            case 'package':
              toolArgs.outputFormat = getOptionValue(parsed.options, 'format', 'zip') as string;
              toolArgs.includeSource = getOptionValue(parsed.options, 'source', true);
              break;
          }

          context.ui.addItem({
            type: MessageType.INFO,
            text: formatInfo(`Performing LaTeX ${operation} on ${projectPath}`)
          }, Date.now());
          ...
          options: [
            {
              ...
              type: 'string',
              choices: ['COMPUTER_SCIENCE', 'BIOLOGY', 'PHYSICS', 'MATHEMATICS', 'ENGINEERING']
            },
            {
              name: 'limit',
            ...
            },
            {
              ...
              type: 'string',
              choices: ['ieee_conference', 'acm_article', 'springer_journal', 'thesis']
            },
            {
              name: 'engine',
