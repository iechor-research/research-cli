...
The integration tests are designed to validate the end-to-end functionality of the Research CLI. They execute the built binary in a controlled environment and verify that it behaves as expected when interacting with the file system.
...
For more detailed debugging, the `--verbose` flag streams the real-time output from the `research` command to the console.
...
... output from the research command ...
...
The workflow runs the tests in different sandboxing environments to ensure Research CLI is tested across each:
