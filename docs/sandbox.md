# Sandboxing in the Research CLI

This document provides a guide to sandboxing in the Research CLI, including prerequisites, quickstart, and configuration.

## Prerequisites

Before using sandboxing, you need to install and set up the Research CLI:

```bash
# install research-cli with npm
...
```bash
DEBUG=1 research -s -p "debug command"
```
...
```bash
# Check environment
research -s -p "run shell command: env | grep SANDBOX"

# List mounts
research -s -p "run shell command: mount | grep workspace"
```
