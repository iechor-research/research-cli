# MCP servers with the Research CLI

This document provides a guide to configuring and using Model Context Protocol (MCP) servers with the Research CLI.

## What is an MCP server?

An MCP server is an application that exposes tools and resources to the Research CLI through the Model Context Protocol, allowing it to interact with external systems and data sources. MCP servers act as a bridge between the Research model and your local environment or other services like APIs.

An MCP server enables the Research CLI to:
...
- **Access resources:** Read data from specific resources (though the Research CLI primarily focuses on tool execution).

With an MCP server, you can extend the Research CLI's capabilities to perform actions beyond its built-in features, such as interacting with databases, APIs, custom scripts, or specialized workflows.

## Core Integration Architecture

The Research CLI integrates with MCP servers through a sophisticated discovery and execution system built into the core package (`packages/core/src/tools/`):
...
4. **Sanitizes and validates** tool schemas for compatibility with the Research API
...
The Research CLI supports three MCP transport types:
...
The Research CLI uses the `mcpServers` configuration in your `settings.json` file to locate and connect to MCP servers. This configuration supports multiple servers with different transport mechanisms.

### Configure the MCP server in settings.json

You can configure MCP servers at the global level in the `~/.research/settings.json` file or in your project's root directory, create or open the `.research/settings.json` file. Within the file, add the `mcpServers` configuration block.
...
When the Research CLI starts, it performs MCP server discovery through the following detailed process:
...
3. **Name sanitization:** Tool names are cleaned to meet Research API requirements:
...
Tool parameter schemas undergo sanitization for Research API compatibility:
...
When the Research model decides to use an MCP tool, the following execution flow occurs:
...
Once discovered, MCP tools are available to the Research model like built-in tools. The model will automatically:
...
- **Property stripping:** The system automatically removes certain schema properties (`$schema`, `additionalProperties`) for Research API compatibility
- **Name sanitization:** Tool names are automatically sanitized to meet API requirements
- **Conflict resolution:** Tool name conflicts between servers are resolved through automatic prefixing

This comprehensive integration makes MCP servers a powerful way to extend the Research CLI's capabilities while maintaining security, reliability, and ease of use.
