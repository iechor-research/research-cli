...
This monorepo contains two main packages: `@iechor/research-cli` and `@iechor/research-cli-core`.

## `@iechor/research-cli`

This is the main package for the Research CLI. It is responsible for the user interface, command parsing, and all other user-facing functionality.

When this package is published, it is bundled into a single executable file. This bundle includes all of the package's dependencies, including `@iechor/research-cli-core`. This means that whether a user installs the package with `npm install -g @iechor/research-cli` or runs it directly with `npx @iechor/research-cli`, they are using this single, self-contained executable.

## `@iechor/research-cli-core`

This package contains the core logic for interacting with the Research API. It is responsible for making API requests, handling authentication, and managing the local cache.
...
Releases are managed through the [release.yml](https://github.com/iechor-research/research-cli/actions/workflows/release.yml) GitHub Actions workflow. To perform a manual release for a patch or hotfix:
...
Every night at midnight UTC, the [Release workflow](https://github.com/iechor-research/research-cli/actions/workflows/release.yml) runs automatically on a schedule. It performs the following steps:
...
```bash
npm install -g @iechor/research-cli@nightly
```

We also run a iEchor cloud build called [release-docker.yml](../.gcp/release-docker.yaml). Which publishes the sandbox docker to match your release. This will also be moved to GH and combined with the main release file once service account permissions are sorted out.

### After the Release

After the workflow has successfully completed, you can monitor its progress in the [GitHub Actions tab](https://github.com/iechor-research/research-cli/actions/workflows/release.yml). Once complete, you should:

1.  Go to the [pull requests page](https://github.com/iechor-research/research-cli/pulls) of the repository.
...
- `npx -y @iechor/research-cli@latest --version` to validate the push worked as expected if you were not doing a rc or dev tag
- `npx -y @iechor/research-cli@<release tag> --version` to validate the tag pushed appropriately
- _This is destructive locally_ `npm uninstall @iechor/research-cli && npm uninstall -g @iechor/research-cli && npm cache clean --force &&  npm install @iechor/research-cli@<version>`
...
1.  Go to the [Actions tab](https://github.com/iechor-research/research-cli/actions/workflows/release.yml) of the repository.
...
You can then inspect the generated tarballs to ensure that they contain the correct files and that the `package.json` files have been updated correctly. The tarballs will be created in the root of each package's directory (e.g., `packages/cli/iechor-research-cli-0.1.6.tgz`).
...
1.  The `package.json` is Transformed:
    ...
    - Why: The final package.json must be different from the one used in development. Key changes include:
      ...
      - Removing workspace-specific "dependencies": { "@research-cli/core": "workspace:\*" } and ensuring the core code is
...
2.  The JavaScript Bundle is Created:
    ...
    - File movement: packages/cli/dist/index.js + packages/core/dist/index.js -> (bundled by esbuild) -> `bundle`/research.js (or a
      similar name).
...
- **Simplified Script Execution**: You can run scripts in any package from the root of the project using the `--workspace` flag. For example, to run the `build` script in the `cli` package, you can run `npm run build --workspace @iechor/research-cli`.
